...
signed main() {
    ...
    if (l.dist(o) > r) { // проверка пересекает ли прямая окружность
        cout << 0 << '\n';
        return 0;
    }

    Vector <ld> n = l.norm(); // берем вектор-нормаль прямой
    n = n.norm(); // нормируем вектор-нормаль
    n = l.dist(o) * n; // делаем длины между окружностью и прямой

    point <ld> h (o.x + n.x, o.y + n.y); // получаем точку
    if (l.a * h.x + l.b * h.y + l.c != 0) { // проверяем лежит ли она на прямой
        n = Vector <ld> (-n.x, -n.y); // если точка не принадлежит прямой, то пересчитываем её
        h = point <ld> (o.x + n.x, o.y + n.y);
    }
    // точка, которую мы получили - это центр окружности, спроектированный на прямую
    if (l.dist(o) == r) { // проверка является ли прямая касательной
        cout << 1 << '\n';
        cout << fixed << setprecision(5) << h << '\n';
        return 0;
    }

    Vector <ld> a = l.direct(); // берем направляющий вектор прямой
    a = a.norm(); // нормируем направляющий вектор прямой
    a = sqrt(r * r - sqr(l.dist(o))) * a; // растягиваем до длины равной расстоянию от данной точки до точки пересечения
    point <ld> i1 (h.x + a.x, h.y + a.y); // считаем первую точку пересечения
    point <ld> i2 (h.x - a.x, h.y - a.y); // считаем вторую точку пересечения
