...
signed main() {
    ...
    if (m.length() < r) { // проверка не лежит ли точка внутри окружности
        cout << 0 << '\n';
        return 0;
    }
    if (m.length() == r) { // проверка лежит ли точка на окружности
        cout << 1 << '\n';
        cout << o2 << '\n';
        return 0;
    }
    ...
    cout << 2 << '\n';
    ld fi = atan2(r, sqrt(sqr(m.length()) - r * r)); // считаем угол между центральной линией и касательной

    Vector <ld> fic = m;
    fic = fic.rotate(fi); // поворачиваем вектор на угол fi
    fic = fic.norm(); // делаем его единичной длины, чтобы потом растянуть
    fic = sqrt(sqr(m.length()) - r * r) * fic; // растягиваем на длину касательной

    point <ld> h1 (fic.x + o2.x, fic.y + o2.y); // считаем точку пересечения касательной с окружностью

    fic = m; // делаем тоже самое, только в другом направлении
    fic = fic.rotate(-fi);
    fic = fic.norm();
    fic = sqrt(sqr(m.length()) - r * r) * fic;
    point <ld> h2 (fic.x + o2.x, fic.y + o2.y);
