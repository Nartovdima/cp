...
bool cmp (point <ld> a, point <ld> b) {
    Vector <ld> m1 (pol[0], a), m2 (pol[0], b);
    if (m1 % m2 == 0)
        return m1.square_length() < m2.square_length();
    else
        return m1 % m2 > 0;
}
...
signed main() {
    ...
    for (int i = 0; i < n; i++) {
        cin >> pol[i];
        if ((pol[i].x < pol[id].x) || (pol[i].x == pol[id].x && pol[i].y < pol[id].y)) 
            id = i;
    }

    swap(pol[0], pol[id]);
    sort(pol.begin() + 1, pol.end(), cmp);
    point <ld> p;
    cin >> p;
    Vector <ld> mn (pol[0], p); 
    ...
    Vector <ld> k (pol[ans - 1], pol[ans]), b (pol[ans - 1], pol[0]), c (pol[ans - 1], p);
    if ((k % b) * (k % c) >= 0)
        cout << "YES" << '\n';
    else
        cout << "NO" << '\n';
